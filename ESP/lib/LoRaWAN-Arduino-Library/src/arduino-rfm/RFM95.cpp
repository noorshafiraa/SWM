...
void RFM_Send_Package(sBuffer *RFM_Tx_Package, sSettings *LoRa_Settings)
{
  ...
  //Wait for TxDone
  #ifdef DEBUG_MAIN_RFM
  Serial.println("Ngirim Data");
  #endif
  uint32_t timetx = millis();
  while (digitalRead(RFM_pins.DIO0) == LOW && (millis() - timetx < 1500));
  #ifdef DEBUG_MAIN_RFM
  Serial.println("Selesai Ngirim");
  #endif
