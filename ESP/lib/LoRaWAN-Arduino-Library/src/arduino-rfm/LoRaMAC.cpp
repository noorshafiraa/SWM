...
{
	static const unsigned int Receive_Delay_1 = 2500;
	static const unsigned int Receive_Delay_2 = 500;
	...
	// wait rx1 window
	#ifdef DEBUG_LORAMAC
	Serial.println("Rx Delay 1"); 
	#endif

	// Modified by jibril to wait for 2,7s in delay 1 22/05/2024
	while ((digitalRead(RFM_pins.DIO0) != HIGH) && (millis() - prevTime < Receive_Delay_1)); // TUnggu DIO0 Low buat selesai ngirim uplink
	#ifdef DEBUG_LORAMAC
	Serial.println("Rx Delay 1 Selesai!"); 
	#endif
...
void LORA_Send_Data(sBuffer *Data_Tx, sLoRa_Session *Session_Data, sSettings *LoRa_Settings)
{
  ...
  Message.Frame_Port = LoRa_Settings->Frame_Port_Tx; 
...
void LORA_Receive_Data(sBuffer *Data_Rx, sLoRa_Session *Session_Data, sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message, sSettings *LoRa_Settings)
{
	...
	{
		/* Added by Jibril 21/05/2024 */
		RFM_Switch_Mode(RFM_MODE_STANDBY);
		RFM_Continuous_Receive(LoRa_Settings);
		uint64_t prevTime_rx = millis();
		#ifdef DEBUG_LORAMAC
		Serial.println("Memulai Rx 1 detik");
		#endif
		while ((millis() - prevTime_rx < 3000) && (digitalRead(RFM_pins.DIO0) != HIGH))
			;

		if (digitalRead(RFM_pins.DIO0) == HIGH){
			#ifdef DEBUG_LORAMAC
			Serial.println("Mendapat rx");
			#endif
			RFM_Switch_Mode(RFM_MODE_STANDBY);
			...
			RFM_Switch_Mode(RFM_MODE_STANDBY);
			#ifdef DEBUG_LORAMAC
			Serial.println("Tidak mendapat rx");
			#endif
	...
	{
		#ifdef DEBUG_LORAMAC
		Serial.println("CRC OK, NOW CHECKING MIC");
		#endif
		...
		if(Message->MAC_Header == 0x40 || Message->MAC_Header == 0x60 || Message->MAC_Header == 0x80 || Message->MAC_Header == 0xA0)
		{
			...
			//Check MIC
			  ...
			  #ifdef DEBUG_LORAMAC
			  Serial.println("MIC OK, NOW CHECKING ADDRESS");
			  #endif
			  ...
			  #ifdef DEBUG_LORAMAC
			  Serial.print("Wrong MIC!, Got MIC count: ");
			  Serial.println(MIC_Check);
			  #endif
			}
			...
			Address_Check = 0;
		  	...
		  	if(Address_Check == 0x04)
		  	{
				...
				#ifdef DEBUG_LORAMAC
				Serial.println("ADDRESS OK, NOW DECRYPTING DATA");
				#endif
		  	...
		  	else
		  	{
				Message_Status = WRONG_MESSAGE;
				#ifdef DEBUG_LORAMAC
				Serial.print("Wrong Address! Address: ");
				Serial.println(Address_Check);
				#endif
			}
			...
			if(Message_Status == ADDRESS_OK)
			{
				...
				if(Data_Rx->Counter != 0x00)
				{
					...
					#ifdef DEBUG_LORAMAC
					Serial.println("MESSAGE DECRYPTED READY TO BE PRINT");
					#endif
				...
				{
					#ifdef DEBUG_LORAMAC
					Serial.println("No data inside Rx");
					#endif
				}
			}
		}
