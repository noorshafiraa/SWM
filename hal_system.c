...
{
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_LEFT_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_UP_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_DOWN_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_SEL_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_RIGHT_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_FORCE_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_LEFT_PORT,
                             HAL_SYS_BUTTON_DISPLAY_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
...
{
    GPIO_clearInterrupt(HAL_SYS_BUTTON_LEFT_PORT, HAL_SYS_BUTTON_LEFT_PIN);
    GPIO_clearInterrupt(HAL_SYS_BUTTON_UP_PORT, HAL_SYS_BUTTON_UP_PIN);
    GPIO_clearInterrupt(HAL_SYS_BUTTON_DOWN_PORT, HAL_SYS_BUTTON_DOWN_PIN);
    GPIO_clearInterrupt(HAL_SYS_BUTTON_SEL_PORT, HAL_SYS_BUTTON_SEL_PIN);
    GPIO_clearInterrupt(HAL_SYS_BUTTON_RIGHT_PORT, HAL_SYS_BUTTON_RIGHT_PIN);
    GPIO_clearInterrupt(HAL_SYS_BUTTON_FORCE_PORT, HAL_SYS_BUTTON_FORCE_PIN);
    GPIO_clearInterrupt(HAL_SYS_BUTTON_DISPLAY_PORT, HAL_SYS_BUTTON_DISPLAY_PIN);
}


void hal_system_ButtonsClearInt(uint16_t buttons)
{
    if (buttons & HAL_SYS_BUTTON_LEFT)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_LEFT_PORT, HAL_SYS_BUTTON_LEFT_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_UP)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_UP_PORT, HAL_SYS_BUTTON_UP_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_DOWN)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_DOWN_PORT, HAL_SYS_BUTTON_DOWN_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_SEL)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_SEL_PORT, HAL_SYS_BUTTON_SEL_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_RIGHT)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_RIGHT_PORT, HAL_SYS_BUTTON_RIGHT_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_FORCE)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_FORCE_PORT, HAL_SYS_BUTTON_FORCE_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_DISPLAY)
    {
        GPIO_clearInterrupt(HAL_SYS_BUTTON_DISPLAY_PORT, HAL_SYS_BUTTON_DISPLAY_PIN);
    }
}
...
void hal_system_ButtonsEnableInt(uint16_t buttons)
{
    ...
    if (buttons & HAL_SYS_BUTTON_LEFT)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_LEFT_PORT, HAL_SYS_BUTTON_LEFT_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_UP)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_UP_PORT, HAL_SYS_BUTTON_UP_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_DOWN)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_DOWN_PORT, HAL_SYS_BUTTON_DOWN_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_SEL)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_SEL_PORT, HAL_SYS_BUTTON_SEL_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_RIGHT)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_RIGHT_PORT, HAL_SYS_BUTTON_RIGHT_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_FORCE)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_FORCE_PORT, HAL_SYS_BUTTON_FORCE_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_DISPLAY)
    {
        GPIO_enableInterrupt(HAL_SYS_BUTTON_DISPLAY_PORT, HAL_SYS_BUTTON_DISPLAY_PIN);
    }
}
...
void hal_system_ButtonsDisableInt(uint16_t buttons)
{
    if (buttons & HAL_SYS_BUTTON_LEFT)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_LEFT_PORT, HAL_SYS_BUTTON_LEFT_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_UP)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_UP_PORT, HAL_SYS_BUTTON_UP_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_DOWN)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_DOWN_PORT, HAL_SYS_BUTTON_DOWN_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_SEL)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_SEL_PORT, HAL_SYS_BUTTON_SEL_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_RIGHT)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_RIGHT_PORT, HAL_SYS_BUTTON_RIGHT_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_FORCE)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_FORCE_PORT, HAL_SYS_BUTTON_FORCE_PIN);
    }
    if (buttons & HAL_SYS_BUTTON_DISPLAY)
    {
        GPIO_disableInterrupt(HAL_SYS_BUTTON_DISPLAY_PORT, HAL_SYS_BUTTON_DISPLAY_PIN);
    }
...
uint16_t hal_system_ButtonsCheck(void)
{
    ...
    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_LEFT_PORT, HAL_SYS_BUTTON_LEFT_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_LEFT;
    }

    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_UP_PORT, HAL_SYS_BUTTON_UP_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_UP;
    }

    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_DOWN_PORT, HAL_SYS_BUTTON_DOWN_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_DOWN;
    }

    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_SEL_PORT, HAL_SYS_BUTTON_SEL_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_SEL;
    }

    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_RIGHT_PORT, HAL_SYS_BUTTON_RIGHT_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_RIGHT;
    }
    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_FORCE_PORT, HAL_SYS_BUTTON_FORCE_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_FORCE;
    }
    if (GPIO_getInterruptStatus(HAL_SYS_BUTTON_DISPLAY_PORT, HAL_SYS_BUTTON_DISPLAY_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_DISPLAY;
    }
...
static void hal_system_GPIOInit(void)
{
     ...
     *  P1[0:1,4] LED0-2 initialized as output Low (High on EVM 1.0)
     *  P1[5] IRQ (OPEN DRAIN)
     *  P1[6:7]  P1.6: UCB0 I2C SDA & P1.7: UCB0 I2C SCL
     *  P2[2:3] VCC_SENSE, set as input
     *  P2[5:6] BUTTON LEFT-UP initialized as input with internal pull-up
     *  P3[0:2] BUTTON DOWN-SELECT-RIGHT initialized as input with internal pull-up
     *  PJ[4:5] LFXT
     */
    ...
    GPIO_setOutputHighOnPin(GPIO_PORT_P1,
                            GPIO_PIN0|GPIO_PIN1|GPIO_PIN4);
#else
    GPIO_setOutputLowOnPin(GPIO_PORT_P1,
                            GPIO_PIN0|GPIO_PIN1|GPIO_PIN4);
#endif
    GPIO_setOutputLowOnPin(GPIO_PORT_P2,
                           GPIO_PIN0|GPIO_PIN1|GPIO_PIN2|GPIO_PIN3|GPIO_PIN4|GPIO_PIN7);
    ...
    GPIO_setAsOutputPin(GPIO_PORT_P7,
                        GPIO_PIN0|GPIO_PIN1|GPIO_PIN2|GPIO_PIN3|GPIO_PIN4|GPIO_PIN5|GPIO_PIN7);
    ...
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P8,GPIO_PIN0);
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P7,GPIO_PIN6);
